...
def main():
    ...
    train_dir = '/root/regis/IXI_data/Train/'
    val_dir = '/root/regis/IXI_data/Val/'
    weights = [1, 1]  # loss weights
    save_dir = 'TransMorph_ncc_{}_diffusion_{}/'.format(weights[0], weights[1])
    if not os.path.exists('experiments_IXI/'+save_dir):
        os.makedirs('experiments_IXI/'+save_dir)
    if not os.path.exists('logs_IXI/'+save_dir):
        os.makedirs('logs_IXI/'+save_dir)
    sys.stdout = Logger('logs_IXI/'+save_dir)
    ...
    if cont_training:
        epoch_start = 136
        model_dir = 'experiments_IXI/'+save_dir
    ...
    best_dsc = 0
    writer = SummaryWriter(log_dir='logs_IXI/'+save_dir)
    for epoch in range(epoch_start, max_epoch):
        ...
        }, save_dir='experiments_IXI/'+save_dir, filename='dsc{:.3f}.pth.tar'.format(eval_dsc.avg))
