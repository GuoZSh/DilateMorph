...
class WindowAttention(nn.Module):
    ...
    def extract_attn_weights(self, query_idx):
        attn_weights = self.attn_weights  # 权重矩阵
        query_weights = attn_weights[query_idx]  # 提取query对应的权重
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W, T):
        ...
        x = self.norm(x)
       # print(self.dim,self.reduce_factor,x.shape, 'line494')
        x = self.reduction(x)
       # print(x.shape,'line495')
    ...
    def forward(self, x, H, W, T):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                ...
                #print(x.shape,'line422')
                x = blk(x, attn_mask)
                #print(x.shape, 'line424')
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        """Forward function."""
       # print(x.shape, 'line670')
        x = self.patch_embed(x)
       # print(x.shape, 'line672')
